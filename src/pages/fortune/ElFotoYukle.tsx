...
const fortuneTellers: Record<string, FortuneTeller> = {
  '1': { id: 1, name: "Hızlı El Okuyucu", cost: 30, emoji: "⚡" },
  '2': { id: 2, name: "Romantik El Falcısı", cost: 40, emoji: "💕" },
  '3': { id: 3, name: "Kariyer El Uzmanı", cost: 50, emoji: "💼" },
  '4': { id: 4, name: "Detaylı Chiromancy Ustası", cost: 65, emoji: "🔍" },
  '5': { id: 5, name: "Holistik El Yorumcusu", cost: 95, emoji: "✨" }
};
...
const ElFotoYukle = () => {
  ...
  const handlePhotoChange = async (type: 'el1' | 'el2' | 'el3', file: File | null) => {
    if (!file) return;
  ...
  const handlePhotoRemove = (type: 'el1' | 'el2' | 'el3') => {
    setPhotos(prev => ({
      ...prev,
      [type]: null
    }));
  ...
  const handleSubmit = async () => {
    ...
    if (uploadedCount < 3) {
      toast({
        ...
        description: "3 fotoğrafı da yüklemelisiniz",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (data.success && data.fortune) {
        ...
        await createNotification(
          user.id,
          '🤚 El Falın Hazır!',
          `${selectedTeller.name} el falını yorumladı. Hemen incele!`,
          'fortune_ready'
        );
              ...
              {!fortune && !loading ? (
                ...
                >
                  ...
                  <h2 className="text-3xl font-bold text-[hsl(220,13%,18%)] mb-4">
                    {selectedTeller.name} elini okuyor... 🤚✨
                  </h2>
                  <p className="text-[hsl(220,9%,46%)]">
                    Çizgiler analiz ediliyor, sabırlı ol...
                  </p>
                </motion.div>
              ) : fortune ? (
                <motion.div
                ...
                >
                  <div className="flex items-center justify-center gap-2 mb-6">
                    ...
                    <h2 className="text-2xl sm:text-3xl font-bold text-foreground font-display">El Falın Yorumu</h2>
                    <Moon className="w-6 h-6 text-accent" />
                  </div>
                  
                  <div className="text-center mb-4">
                    <span className="text-4xl">{selectedTeller.emoji}</span>
                    <p className="text-muted-foreground mt-2">{selectedTeller.name}</p>
                  </div>

                  {previews.el1 && (
                    <div className="mb-6 rounded-2xl overflow-hidden bg-card/30 backdrop-blur-sm border border-accent/20 shadow-lg">
                      <img
                        src={previews.el1}
                        alt="El fotoğrafı"
      ...
      >
        {preview ? (
          ...
          >
            ...
            <p className="text-xs text-[hsl(220,9%,46%)] px-2 text-center">
              {field.description}
            </p>
            <p className="text-xs text-[hsl(258,90%,76%)] font-medium mt-2">
              + Ekle
            </p>
          </button>
        )}
